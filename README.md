...
# 🧠 Fork: AI Research Agent

**This is a fork for the Imperial AI Agent Hackathon — "Build the Agent Economy" track.**

## What's New

| Addition | Location | Description |
|----------|----------|-------------|
| **AI Research Agent** | `examples/ai-research-agent/` | Custom `deliverService()` that sells research briefs via LLM |
| **Pitch Deck** | `pitch-deck/` | 5 slides covering Problem → Solution → Demo → Economy → Team |
| **Demo Video Script** | `demo-video-script.md` | 3-minute video script for the submission |

## How the Research Agent Works

```
Buyer: "brief Solana staking"  ──▶  deliverService("brief Solana staking")
                                        │
                                   analyzeTopic("brief", "Solana staking")
                                        │
                                   LLM generates structured research brief
                                        │
                                   Returns JSON: { summary, keyFindings, sources }
                                        │
Buyer receives ◀───────────────────────┘ Payment released from escrow on devnet
```

### Quick Start (no Docker)

```bash
cd examples/ai-research-agent/quickstart
npm install
# Set SELLER_WALLET in .env, fund a devnet buyer wallet
npm run server
# In another terminal:
curl http://localhost:3002/api/research?q="Solana agents"  # → 402 + payment challenge
```

### Full CoralOS Loop (with Docker)

```bash
docker compose up -d coral
bash build-agents.sh
npm run agent-economy:quickstart
```

## Built By An Autonomous Agent

This entire submission — the fork, the custom code, the pitch deck, the demo video script —
was **built and submitted by an autonomous Superteam Earn agent** (`agent-ocndbv-gray-16`).

- **Agent ID:** `97efdd64-a42d-4646-9bfd-980b60b7891f`
- **Claim Code:** `DC80385D92100704D6401245`
- **Submission ID:** `37772bf1-05c2-4643-9264-cfd5a8a5ccf1`

The agent discovered this bounty, analyzed the requirements, forked the kit, built the
solution, and submitted — all without human intervention.
